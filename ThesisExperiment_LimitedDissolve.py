...
samples_per_face = 3
angle_deg = 40
angle_rad = math.radians(angle_deg)z
...
def sample_and_measure(src_obj, tgt_obj, samples_per_face, depsgraph):
    # Build BVH from evaluated target
...
def create_face_material(obj):
    mat = bpy.data.materials.new(name="FaceHeatmap")
    mat.use_nodes = True
...
for file_name in os.listdir(input_folder):
    ...
    log_file_path = os.path.join(log_file_folder, (file_name[:-4] + f"_Log_LD.txt"))
    ...
    # Skip if no mesh in imported model
    imported = [obj for obj in bpy.context.selected_objects if obj.type == 'MESH']
    if not imported:
        print(f"Skipping {file_name}: no mesh")
        continue

    # if import contains multiple meshes join them, skip if fail
    ...
    # get vertex- edge- and face- count before simplification
    v1, e1, f1 = get_mesh_stats([reference])

    # Perform limited dissolve simplification
    bpy.ops.object.select_all(action='DESELECT')
    original.select_set(True)
    bpy.context.view_layer.objects.active = original
    ...
    bpy.ops.mesh.dissolve_limited(angle_limit=angle_rad)
    ...
    # get vertex- edge- and face- count after simplification
    v2, e2, f2 = get_mesh_stats([original])

    # sample points, find distances, calculate hausdorff- and RMS distance
    ...
    # create heatmap material and apply it to simplified mesh
    ...
    create_face_material(original)

    # Export simplified mesh
    export_name = os.path.splitext(file_name)[0] + "_LD.obj"
